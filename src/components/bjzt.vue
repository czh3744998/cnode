    ...
    <div id="content">
      <div class="panel">
        ...
        <div class="inner post">
          ...
          <form id="create_topic_form"  method="post" v-on:submit.prevent="setdata">

            <fieldset>
              <span class="tab-selector">选择版块：</span>
              <select name="tab" id="tab-value" :value="tab">
              ...
              <textarea autofocus="" class="span9" id="title" name="title" rows="1" placeholder="标题字数 10 字以上" v-model="title"></textarea>

              <textarea name="" id="" cols="84" rows="15" v-model="mesg"  v-html="mesg"></textarea>
              <input type="submit" style="width: 80px;height: 40px;background: deepskyblue;color: white;border: 0;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;" @click="getdata()">
              <input type="hidden" id="topic_tags" name="topic_tags" value="">
              <input type="hidden" name="_csrf" value="1CVIhDL7-gQ9O3eloknUNerIONFOHycmtH0I">
            </fieldset>
          </form>
        </div>
    ...
    export default {
      ...
      methods:{
        setdata(){
          ...
          // console.log(id)
          // console.log("https://cnodejs.org/api/v1/topic/"+this.id)
          this.$axios.get("https://cnodejs.org/api/v1/topic/"+id).then(res=>{
            this.arr = res.data.data
            console.log(this.arr)
            this.mesg=res.data.data.content
...
<style scoped lang="scss">
  ...
  #create_topic_form textarea.editor, #edit_reply_form textarea.editor {
    height: 400px;
  }
  textarea.editor {
    width: 98%;
  ...
  .eicon-bold, .eicon-code, .eicon-contract, .eicon-fullscreen, .eicon-image, .eicon-info, .eicon-italic, .eicon-link, .eicon-music, .eicon-ordered-list, .eicon-play, .eicon-preview, .eicon-question, .eicon-quote, .eicon-redo, .eicon-undo, .eicon-unordered-list {
    font-family: icomoon;
    speak: none;
    font-style: normal;
    font-weight: 400;
